...
class LLM4Rec(nn.Module):
    def __init__(self, **args):
        ...
        self.llama_tokenizer = LlamaTokenizer.from_pretrained(self.args['base_model'], use_fast=False, local_files_only=False, cache_dir=args['cache_dir'])
    ...
    def predict(self, inputs, inputs_mask):
        ...
        # Using .to(self.llama_model.device) ensures it uses MPS on Mac or CPU automatically
        instruct_embeds = self.llama_model.model.embed_tokens(self.instruct_ids.to(self.llama_model.device)).expand(bs, -1, -1)
        response_embeds = self.llama_model.model.embed_tokens(self.response_ids.to(self.llama_model.device)).expand(bs, -1, -1)
        # instruct_mask = self.instruct_mask.cuda().expand(bs, -1)
        # response_mask = self.response_mask.cuda().expand(bs, -1)
        instruct_mask = self.instruct_mask.to(self.llama_model.device).expand(bs, -1)
        response_mask = self.response_mask.to(self.llama_model.device).expand(bs, -1)
