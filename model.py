...
class LLM4Rec(nn.Module):
    def __init__(self, **args):
        ...
        raw_embeds = self.args['input_embeds']
        if isinstance(raw_embeds, list):
            # Multi-modality path: trainable FusionModule lives inside the model.
            from utils.fusion import FusionModule
            self.embeds_list = nn.ModuleList([
                nn.Embedding.from_pretrained(e, freeze=True).to(device) for e in raw_embeds
            ])
            self.input_embeds = None
            self.fusion = FusionModule(
                modality_dims=[e.shape[1] for e in raw_embeds],
                strategy=self.args.get('fusion_strategy', 'concat'),
                output_dim=self.args.get('fusion_target_dim') or raw_embeds[0].shape[1],
            ).to(device)
            proj_in = self.fusion.output_dim
        else:
            # Single pre-fused tensor path (zero-shot precompute or concat finetuning).
            self.input_embeds = nn.Embedding.from_pretrained(raw_embeds, freeze=True).to(device)
            self.embeds_list = None
            self.fusion = None
        ...
        # to avoid materialising a [hidden_size × n_items] tensor on GPU every step.
