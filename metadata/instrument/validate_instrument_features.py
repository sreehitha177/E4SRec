#!/usr/bin/env python3
"""
Annotate validation tracks for all instrument features, then evaluate vs MGPHot.
"""
...
def evaluate_annotations(df: pd.DataFrame, features: list[str]) -> pd.DataFrame:
    ...
    for feature in features:
        idx = INSTRUMENTAL_FEATURE_INDICES[feature]
...
def print_bias(validation_with_preds: pd.DataFrame, features: list[str]) -> None:
    ...
    for feature in features:
        ...
        print(f"{feature:<30} {gt.mean():>8.3f} {pred.mean():>10.3f} {bias:>8.3f}")
...
def plot_heatmap(eval_df: pd.DataFrame, out_dir: str) -> None:
    ...
    plt.tight_layout()
    path = os.path.join(out_dir, "validation_instrument_metrics_heatmap.png")
...
def plot_distributions(validation_with_preds: pd.DataFrame, features: list[str], out_dir: str) -> None:
    ...
    plt.tight_layout()
    path = os.path.join(out_dir, "validation_instrument_distributions.png")
    plt.savefig(path, dpi=150)
    print(f"Saved {path}")
    plt.close()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-csv",
        default=str(_PKG_DIR / "output" / "validation_instrument_annotations.csv"),
    )
...
def main() -> None:
    ...
    if args.plots_only:
        ...
        annotations = annotate_df_parallel(
            ...
            selected_features=INSTRUMENTAL_FEATURES,
    ...
    eval_results = evaluate_annotations(validation_with_preds, INSTRUMENTAL_FEATURES)
    print("\n=== Evaluation Results ===")
    ...
    print("\n=== Bias Check ===")
    print_bias(validation_with_preds, INSTRUMENTAL_FEATURES)

    plots_dir = _PKG_DIR / "plots"
    plots_dir.mkdir(parents=True, exist_ok=True)
    plot_heatmap(eval_results, out_dir=str(plots_dir))
    plot_distributions(validation_with_preds, INSTRUMENTAL_FEATURES, out_dir=str(plots_dir))
