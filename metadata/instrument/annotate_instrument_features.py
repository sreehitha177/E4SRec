#!/usr/bin/env python3
"""
Annotate selected instrument features for all songs in ordered list.
...
def load_seed_annotations(seed_csv: str | None, selected_features: list[str]) -> pd.DataFrame:
    ...
    # Keep only keys + requested feature columns. Missing feature columns are filled with NaN.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument(
        "--output-csv",
        default=str(_PKG_DIR / "output" / "instrumental_annotations_selected_features.csv"),
    )
    common.add_argument(
        "--num-nodes",
        type=int,
