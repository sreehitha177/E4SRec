#!/usr/bin/env python3
"""
Annotate validation tracks for all vocal features, then evaluate vs MGPHot.
"""
...
def evaluate_annotations(df: pd.DataFrame, features: list[str]) -> pd.DataFrame:
    ...
    for feature in features:
        idx = VOCAL_FEATURE_INDICES[feature]
...
def plot_heatmap(eval_df: pd.DataFrame, out_dir: str) -> None:
    ...
    plt.tight_layout()
    path = os.path.join(out_dir, "validation_vocal_metrics_heatmap.png")
...
def plot_distributions(validation_with_preds: pd.DataFrame, features: list[str], out_dir: str) -> None:
    ...
    axes[-1].axis("off")
    plt.tight_layout()
    path = os.path.join(out_dir, "validation_vocal_distributions.png")
    plt.savefig(path, dpi=150)
    print(f"Saved {path}")
    plt.close()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-csv",
        default=str(_PKG_DIR / "output" / "validation_vocal_annotations.csv"),
    )
...
def main() -> None:
    ...
    if args.plots_only:
        ...
        annotations = annotate_df_parallel(
            ...
            selected_features=VOCAL_FEATURES,
    ...
    eval_results = evaluate_annotations(validation_with_preds, VOCAL_FEATURES)
    print("\n=== Evaluation Results ===")
    ...
    print("\n=== Bias Check ===")
    print_bias(validation_with_preds, VOCAL_FEATURES)

    plots_dir = _PKG_DIR / "plots"
    plots_dir.mkdir(parents=True, exist_ok=True)
    plot_heatmap(eval_results, out_dir=str(plots_dir))
    plot_distributions(validation_with_preds, VOCAL_FEATURES, out_dir=str(plots_dir))
