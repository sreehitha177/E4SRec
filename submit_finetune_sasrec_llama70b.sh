#!/bin/bash
# submit_finetune_sasrec_llama70b.sh
#
# Submits all 39 SASRec finetuning experiments for Llama-3-70B:
#   13 fusion combos × 3 completion modes (none, prompt, embed)
# Each job requests 1x A100 80GB.
#
# Usage:
#   bash submit_finetune_sasrec_llama70b.sh           # submit all 39
#   bash submit_finetune_sasrec_llama70b.sh --dry-run
...
CHECKPOINT_BASE="/scratch3/workspace/snarayana_umass_edu-checkpoints/SASRec_Llama-3-70B"
...
submit() {
    ...
    local output_dir="${CHECKPOINT_BASE}/SASRec"
    [ -n "$audio" ] || [ -n "$lyric" ] && output_dir="${CHECKPOINT_BASE}/SASRec_$([ -n "$audio" ] && echo -n "audio")$([ -n "$audio" ] && [ -n "$lyric" ] && echo -n "_")$([ -n "$lyric" ] && echo -n "lyric")_${fusion}"
    [ "$cr_mode" != "none" ] && output_dir="${output_dir}_completion_${cr_mode}"

    local job_script=$(cat <<EOF
#!/bin/bash
#SBATCH --job-name=ftSAS70b_${tag}
...
#SBATCH --qos=long
#SBATCH --output=logs/ftSAS70b_${tag}_%j.log
#SBATCH --error=logs/ftSAS70b_${tag}_%j.err
...
EOF
    ...
    echo "  Submitting: ftSAS70b_${tag}"
    if [ "$DRY_RUN" -eq 0 ]; then
        echo "$job_script" | sbatch
    else
        echo "  [dry-run] would submit: ftSAS70b_${tag}"
    fi
}
...
echo "=========================================="
echo " Submitting 26 SASRec finetune experiments (Llama-3-70B, A100)"
echo "=========================================="
mkdir -p logs
