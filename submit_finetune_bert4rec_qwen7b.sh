...
#
# Usage:
#   bash submit_finetune_bert4rec_qwen7b.sh           # submit all 39
#   bash submit_finetune_bert4rec_qwen7b.sh --dry-run
...
submit() {
    ...
    local micro=2
    if [ -n "$lyric" ]; then
        gres="gpu:1 --constraint=vram48"
        micro=1
    fi

    local job_script=$(cat <<EOF
#!/bin/bash
#SBATCH --job-name=ftB4R7b_${tag}
...
EOF
    ...
    echo "  Submitting: ftB4R7b_${tag}"
    if [ "$DRY_RUN" -eq 0 ]; then
        echo "$job_script" | sbatch
    else
        echo "  [dry-run] would submit: ftB4R7b_${tag}"
    fi
}
...
echo "=========================================="
echo " Submitting 39 BERT4Rec finetune experiments (Qwen2.5-7B)"
echo "=========================================="
mkdir -p logs
