...
# Option A: stacked labels above the cluster, sorted by value (ascending = bottom of stack).
# All labels sit above every line; arrows connect each label to its data point.
BASE_OFFSET = 16   # pts above the lowest-ranked point
STACK_STEP  = 14   # pts between consecutive labels in the stack
...
def make_line_chart(table_name, llm):
    ...
    for row, col, mi, metric_label in PANELS:
        ...
        for xi in range(len(CONFIGS)):
            ...
            for rank, (val, si) in enumerate(pts):
                y_off = BASE_OFFSET + rank * STACK_STEP
    ...
    fig.tight_layout()
    slug = f"line_{table_name.replace(' ', '')}_{llm.replace('.', '').replace('-', '')}_optA"
