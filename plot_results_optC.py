...
# Option C: wide horizontal spread per series, no arrows.
# SASRec center-above, BERT4Rec far-left, GRU4Rec far-right.
# Horizontal distance (~30 pts) ensures no overlap even when values are identical.
OFFSETS = [(0, 14), (-30, 10), (30, 10)]
...
def make_line_chart(table_name, llm):
    ...
    fig.tight_layout()
    slug = f"line_{table_name.replace(' ', '')}_{llm.replace('.', '').replace('-', '')}_optC"
