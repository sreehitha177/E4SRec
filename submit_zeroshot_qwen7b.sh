#!/bin/bash
# submit_zeroshot_qwen7b.sh
#
# Submits zero-shot experiments for Qwen2.5-7B-Instruct covering:
...
#
# Usage:
#   bash submit_zeroshot_qwen7b.sh           # submit all 117
#   bash submit_zeroshot_qwen7b.sh --dry-run  # print without submitting

DRY_RUN=0
[ "${1}" = "--dry-run" ] && DRY_RUN=1

BASE_MODEL="/datasets/ai/qwen2/hub/models--Qwen--Qwen2.5-7B-Instruct/snapshots/a09a35458c702b33eeacc393d103063234e8bc28"
CACHE_DIR="/datasets/ai/qwen2/hub"
...
#SBATCH --output=logs/zs7b_${tag}_%j.log
#SBATCH --error=logs/zs7b_${tag}_%j.err
...
echo "Job: zs7b_${tag}  |  Node: \$(hostname)  |  GPU: \$CUDA_VISIBLE_DEVICES"
...
EOF
    ...
    echo "  Submitting: zs7b_${tag}"
    if [ "$DRY_RUN" -eq 0 ]; then
        echo "$job_script" | sbatch
    else
        echo "  [dry-run] would submit: zs7b_${tag}"
    fi
}
...
echo "============================================================"
echo " Submitting 117 zero-shot experiments (Qwen2.5-7B)"
echo "============================================================"
