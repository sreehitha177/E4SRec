#!/bin/bash
# submit_finetune_gru4rec_llama70b.sh
#
# Submits all 26 GRU4Rec finetuning experiments for Llama-3-70B:
#   13 fusion combos × 2 completion modes (none, prompt)
# Each job requests 1x A100 80GB.
#
# Usage:
#   bash submit_finetune_gru4rec_llama70b.sh           # submit all 26
#   bash submit_finetune_gru4rec_llama70b.sh --dry-run
...
CHECKPOINT_BASE="/scratch3/workspace/snarayana_umass_edu-checkpoints/GRU4Rec_Llama-3-70B"
GRU4REC_EMBED="/work/pi_dagarwal_umass_edu/project_7/srikar/E4SRec/datasets/sequential/LastFM/GRU4Rec_item_embed.pkl"
...
submit() {
    ...
    local output_dir="${CHECKPOINT_BASE}/GRU4Rec"
    [ -n "$audio" ] || [ -n "$lyric" ] && output_dir="${CHECKPOINT_BASE}/GRU4Rec_$([ -n "$audio" ] && echo -n "audio")$([ -n "$audio" ] && [ -n "$lyric" ] && echo -n "_")$([ -n "$lyric" ] && echo -n "lyric")_${fusion}"
    [ "$cr_mode" != "none" ] && output_dir="${output_dir}_completion_${cr_mode}"

    local job_script=$(cat <<EOF
#!/bin/bash
#SBATCH --job-name=ftG4R70b_${tag}
...
#SBATCH --qos=long
#SBATCH --output=logs/ftG4R70b_${tag}_%j.log
#SBATCH --error=logs/ftG4R70b_${tag}_%j.err
...
EOF
    ...
    echo "  Submitting: ftG4R70b_${tag}"
    if [ "$DRY_RUN" -eq 0 ]; then
        echo "$job_script" | sbatch
    else
        echo "  [dry-run] would submit: ftG4R70b_${tag}"
    fi
}
...
echo "=========================================="
echo " Submitting 26 GRU4Rec finetune experiments (Llama-3-70B, A100)"
echo "=========================================="
mkdir -p logs
