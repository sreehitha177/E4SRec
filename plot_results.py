...
# ── Data ──────────────────────────────────────────────────────────────────────
# data[table][llm][seq_model][config] = [R5, N5, R10, N10, R20, N20]
CONFIGS    = ["Only", "+ Audio/\nLyric/Meta", "+ A/L/M +\nCR"]
SEQ_MODELS = ["SASRec", "BERT4Rec", "GRU4Rec"]
LLMS       = ["Qwen2.5-7B", "Llama-2-13B", "Llama-3-70B"]

R10, N10 = 2, 3   # indices into the 6-element metric list
...
# ── Style ─────────────────────────────────────────────────────────────────────
COLORS    = ["#4C72B0", "#DD8452", "#55A868"]   # SASRec, BERT4Rec, GRU4Rec
MARKERS   = ["o", "s", "^"]
LINESTYLE = ["-", "--", ":"]
...
# 2×3 grid: row0 = Recall @5/10/20, row1 = NDCG @5/10/20
PANELS = [
    (0, 0, 0, "Recall@5"),
    (0, 1, 2, "Recall@10"),
    (0, 2, 4, "Recall@20"),
...
def make_line_chart(table_name, llm):
    ...
    fig.suptitle(f"{llm}", fontsize=16, fontweight="bold", y=1.01)
    ...
    for row, col, mi, metric_label in PANELS:
        ...
        for seq_idx, (seq, color, marker, ls) in enumerate(zip(SEQ_MODELS, COLORS, MARKERS, LINESTYLE)):
            ...
            # Different offsets for each series to avoid overlap
            offsets = [(0, 12), (-8, 8), (8, 8)]
            xytext = offsets[seq_idx]
    ...
    fig.tight_layout()
    slug = f"line_{table_name.replace(' ', '')}_{llm.replace('.', '').replace('-', '')}"
