...
OUTPUT_PATH = "/work/pi_dagarwal_umass_edu/project_7/swetha/harmony_annotations_remaining.csv"
...
harmony_features = pd.read_csv(GENES_PATH, sep="\t").iloc[7:9]
HARMONY_FEATURES = harmony_features["name"].tolist()

FEATURE_DEFINITIONS = {
    "Minor / Major Key Tonality": "0=minor, 5=major, 2-3=mixed/ambiguous",
    "Harmonic Sophistication": "0=I/IV/V, 1=diatonic, 2=secondary dominants, 3=non-diatonic, 4=chromatic, 5=high chromatic",
}
...
def build_user_prompt(artist: str, title: str) -> str:
    ...
    keys_str = ", ".join(HARMONY_FEATURES)

    return (
        f'Track="{title}" Artist="{artist}". '
        f'Rate these harmony features 0-5 using music knowledge only. '
        f'{feature_block}. '
...
def validate_config():
    if not API_KEY:
        raise RuntimeError(
            "Missing LITELLM_API_KEY. Set it first, for example:\n"
...
def preflight_check(df, artist_col="artist_name", title_col="track_name"):
    ...
    missing = [feat for feat in HARMONY_FEATURES if feat not in result or result[feat] is None]
    if missing:
...
#         "artist_name", "track_id", "track_name", "session_id"
#     ]
...
if __name__ == "__main__":
    ...
    # LIMIT TO FIRST 10 TRACKS (SAFE TEST RUN)
    # remaining_df = remaining_df.head(3)

    print(f"\nRunning only on first {len(remaining_df)} tracks (TEST RUN)")
    
    if remaining_df.empty:
        print("No tracks left to annotate.")
        raise SystemExit(0)

    # One-call hard stop before any bulk processing
    preflight_check(remaining_df)

    # Single-row safety check after preflight
    ...
    # Main batches, still fully sequential inside each batch
