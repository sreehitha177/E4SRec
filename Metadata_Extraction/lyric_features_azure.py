...
# Files
OUTPUT_PATH = "/work/pi_dagarwal_umass_edu/project_7/swetha/Metadata_Extraction/outputs/lyrics_annotations_remaining.csv"
...
lyrics_genes = pd.read_csv(GENES_PATH, sep="\t").iloc[35:43]
LYRICS_FEATURES = lyrics_genes["name"].tolist()

FEATURE_DEFINITIONS = {
    "Angry Lyrics": (
        "0 none, 1 mild irritation, 2 resentment/confrontation, "
        "3 recurring anger, 4 intense rage, 5 anger throughout"
    ),
    "Sad Lyrics": (
        "0 none, 5 sadness dominant throughout"
    ),
    "Happy/Joyful Lyrics": (
        "0 none, 5 joy dominant throughout"
    ),
    "Humorous Lyrics": (
        "0 none, 5 humor dominant throughout"
    ),
    "Love/Romance Lyrics": (
        "0 none, 5 love/romance dominant throughout"
    ),
    "Social/Political Lyrics": (
        "0 purely personal, 1 passing social reference, 2 secondary social theme, "
        "3 clear message, 4 primarily political, 5 protest anthem"
    ),
    "Abstract Lyrics": (
        "0 fully literal, 1 occasional metaphor, 2 notably symbolic, "
        "3 abstract imagery dominant, 4 surreal, 5 entirely dreamlike/non-literal"
    ),
    "Explicit Lyrics": (
        "0 none, 5 highly explicit throughout"
    ),
}

SYSTEM_PROMPT = (
    "Rate lyrical features of a song. "
...
def build_user_prompt(artist: str, title: str) -> str:
    ...
    keys_str = ", ".join(LYRICS_FEATURES)

    return (
        f'Track="{title}" Artist="{artist}". '
        f'Rate these lyrics features 0-5 using music knowledge only. '
        f'{feature_block}. '
...
def preflight_check(df, artist_col="artist_name", title_col="track_name"):
    ...
    missing = [feat for feat in LYRICS_FEATURES if feat not in result or result[feat] is None]
    if missing:
...
def annotate_df_safe(
    ...
    for i, row in unique_tracks.iterrows():
        ...
        entry.update(scores if scores else {feat: None for feat in LYRICS_FEATURES})
