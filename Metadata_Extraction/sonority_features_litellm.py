...
OUTPUT_PATH = "/work/pi_dagarwal_umass_edu/project_7/swetha/sonority_annotations_remaining.csv"
...
sonority_features = pd.read_csv(GENES_PATH, sep="\t").iloc[43:49]
...
SYSTEM_PROMPT = (
    "Rate sonority features of a song. "
...
def preflight_check(df, artist_col="artist_name", title_col="track_name"):
    ...
    missing = [feat for feat in SONORITY_FEATURES if feat not in result or result[feat] is None]
    if missing:
...
if __name__ == "__main__":
    ...
    # For test runs, uncomment one of these:
    # remaining_df = remaining_df.head(3)
    # remaining_df = remaining_df.head(10)
    # remaining_df = remaining_df.head(5000)

    print(f"\nRunning on {len(remaining_df)} tracks")
